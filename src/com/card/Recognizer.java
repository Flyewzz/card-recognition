...
import com.card.services.ImageRecognizerService;
...
public class Recognizer {
        ...
        return Arrays.stream(templs).collect(Collectors
                .toMap(s->s.replaceFirst("[.][^.]+$", ""), //Избавляемся от расширения в имени файла
                        s->{
                            try
                            {
                                return ImageIO.read(Recognizer.class.getClassLoader().getResourceAsStream(s));
                            } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        for (File inFile : arrlf) {
            ...
            for (int cardNum = 0; cardNum < Constants.ARRAY_OF_CARD_POSITION_X.length; cardNum++) {
                ...
                for (Map.Entry<String, BufferedImage> entry : mapRankTempls.entrySet()) {
                    ...
                    int pd = ImageRecognizerService.getCompareIndexOfSubImages(img1, img2,
        ...
        System.out.println();
        System.out.printf("Всего распознано карт: %d \n", totalFoundCards);
        System.out.printf("Всего нераспознанных карт: %d \n", totalNotFoundCards);
    }
    }
}
